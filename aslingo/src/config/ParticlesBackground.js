...
const GalaxyBackground = (props) => {
  ...
  const starCount = props.starCount || 150;
  const rotationSpeed = props.rotationSpeed || 0.01;
  const innerRadius = props.innerRadius || 100;
  const minSize = props.minSize || 0.5;
  const maxSize = props.maxSize || 1.8;
  const opacity = props.opacity || 0.9;
  const bgColor = (props.bgColor && isValidHexCode(props.bgColor)) ? props.bgColor : "#000000";
  const starColor = (props.starColor && isValidHexCode(props.starColor)) ? props.starColor : "#ffffff";
